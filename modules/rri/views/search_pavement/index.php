<form class="form-horizontal" role="form" action="<?= site_url('rri/search_pavement/pdf_') ?>" method="post"> 
    <div class="panel panel-default">

        <div class="panel-heading">
            <h3 class="panel-title"><img src="<?= base_url('assets/bs/css/images/search.gif') ?>" />​&nbsp;<?= t('ស្វែងរកតាមប្រភេទកម្រាល') ?></h3>
        </div>
        ...
        <div class="panel-body">  
            ...
            <div style="margin: -10px 0 40px 0;vertical-align: middle;">
                ...
                <span style="float: right;">                    
                    <label for="pavement"><?= t('ស្វែងរកតាមកម្រាល') ?></label>
                    <select validate_act name="pavement" id="pavement" class="pavement" style="width: 150px;height: 30px;"></select> 

                    <button type="button" id="btn_prt" class="btn_prt" style="height: 30px;">បោះពុម្ព</button>
                    <button type="submit" style="height: 30px;">PDF</button>                    
                </span>
            </div>
            ...
            <table cellpadding="0" cellspacing="0" border="1" class="table table-striped table-bordered table-hover" id="my_gr">
                <thead>
                    <tr>
                        <th style="text-align: center;width: 8%;"><a href="javascript: void(0)" class="gis_id" id="gis_id" data-direction="asc"><?= t('GIS ID') ?></a></th>
                        <th style="text-align: center;"><?= t('លេខ​ផ្លូវ') ?></th>
                        <th style="text-align: center;width: 40%;"><?= t('ឈ្មោះ​​ផ្លូវ') ?></th>
...
<script type="text/javascript">
    ...
    // get num. display ============
    $.ajax({
        url: '<?= site_url('rri/search_pavement/get_num_dip') ?>',
        type: 'post',
        datatype: 'json',
        async: false,
        beforeSend: function() {
    ...
    // get pavement ================
    $.ajax({
        url: '<?= site_url('rri/search_pavement/opt_pavement') ?>',
        type: 'post',
        datatype: 'json',
        async: false,
        beforeSend: function() {
        ...
        success: function(data) {
            if (data.length > 0) {
                ...
                opt += '<option></option>';
                $.each(data, function(i, item) {
                    opt += '<option value="'+ item.type_pavement +'">' + item.type_pavement + '</option>';
                });
            }
            $('#pavement').html(opt);
            $('.xmodal').hide();
    ...
    function print_dt() {
        var pavement = $('#pavement').val();
        $.ajax({
            url: '<?= site_url('rri/search_pavement/print_pavement') ?>',
            type: 'post',
            datatype: 'json',
            async: false,
            beforeSend: function() {
    ...
    function grid(current_page, total_display) {
        ...
        $.ajax({
            url: '<?= site_url('rri/search_pavement/grid_pavement') ?>',
            type: 'post',
            datatype: 'json',
            async: true,
            beforeSend: function() {
