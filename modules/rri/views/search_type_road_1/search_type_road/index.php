        ...
        <div class="panel-body">  
            ...
            <div style="margin: -10px 0 40px 0;">
                ...
                <span style="float: right;">                    
                    <label for="type"><?= t('ស្វែងរកតាមប្រភេទ') ?></label>
                    <select id="type" class="type" style="width: 80px;height: 30px;"></select> 
...
<script type="text/javascript">
    $(function() {
        ...
        $('body').delegate('#gis_id', 'click', function() {
            $(this).attr('data-direction') === 'ASC' ? $(this).attr('data-direction', 'DESC') : $(this).attr('data-direction', 'ASC');
        ...
        // get num. display ===========
        $.ajax({
            url: '<?= site_url('rri/search_type_road/get_num_dip') ?>',
            type: 'post',
            datatype: 'json',
            async: false,
            beforeSend: function() {
        ...
        // get type ================
        $.ajax({
            url: '<?= site_url('rri/search_type_road/opt_type') ?>',
            type: 'post',
            datatype: 'json',
            async: false,
            beforeSend: function() {
    ...
    function print_dt() {
        var type = $('#type').val();
        $.ajax({
            url: '<?= site_url('rri/search_type_road/get_by_type') ?>',
            type: 'post',
            datatype: 'json',
            async: false,
            beforeSend: function() {
    ...
    function grid(current_page, total_display) {
        ...
        var direction = $('#gis_id').attr('data-direction');

        $.ajax({
            url: '<?= site_url('rri/search_type_road/grid_type_road') ?>',
            type: 'post',
            datatype: 'json',
            // async: false,
            beforeSend: function() {
            ...
            data: {offset: offset, limit: limit, type: type, direction: direction
            },
            success: function(data) {
