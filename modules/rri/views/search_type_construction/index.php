<form class="form-horizontal" action="<?= site_url('rri/search_type_construction/pdf') ?>" role="form" method="post"> 
    <div class="panel panel-default">

        <div class="panel-heading">
            <h3 class="panel-title"><img src="<?= base_url('assets/bs/css/images/search.gif') ?>" />​ <?= t('ស្វែងរកតាមសំណង់សាធារណៈ') ?></h3>
        </div>
        ...
        <div class="panel-body">  
            ...
            <div style="margin: -10px 0 40px 0;vertical-align: middle;">
                ...
                <span style="float: right;">                    
                    <label for="type_building"><?= t('ស្វែងរកតាមសំណង់') ?></label>
            ...
            <table cellpadding="0" cellspacing="0" border="1" class="table table-striped table-bordered table-hover" id="my_gr">
                <thead>
                    <tr>
                        <th style="text-align: center;width: 20%;"><?= t('ប្រភេទសំណង់') ?></th>
...
<script type="text/javascript">
    ...
    // get type_building ==============
    $.ajax({
        url: '<?= site_url('rri/search_type_construction/opt_building') ?>',
        type: 'post',
        datatype: 'json',
        // async: false,
        beforeSend: function() {
        ...
        success: function(data) {
            if (data.length > 0) {
                ...
                opt += '<option></option>';
                $.each(data, function(i, item) {
                    opt += '<option value="' + item.type_building + '">' + item.type_building + '</option>';
                });
            }
    ...
    function print_dt() {
        var type_building = $('#type_building').val();
        $.ajax({
            url: '<?= site_url('rri/search_type_construction/print_building') ?>',
            type: 'post',
            datatype: 'json',
            async: false,
            beforeSend: function() {
            ...
            success: function(data) {
                ...
                if (data.length > 0) {
                    $.each(data, function(i, row) {
                        ...
                        tr += "<tr>" +
                                "<td>" + i + "</td>" +
                                "<td style='text-align: left;'>" + (row.type_building !== '' ? row.type_building : '') + "</td>" +
                                "<td style='text-align: left;'>" + (row.name_building !== '' ? row.name_building : '') + "</td>" +
    ...
    function grid(current_page, total_display) {
        ...
        $.ajax({
            url: '<?= site_url('rri/search_type_construction/grid_type_construction') ?>',
            type: 'post',
            datatype: 'json',
            // async: false,
            beforeSend: function() {
            ...
            success: function(data) {
                ...
                if (record.length > 0) {
                    $.each(record, function(i, row) {
                        i++;
                        tr += "<tr>" +
                                "<td style='text-align: left;'>" + row.type_building + "</td>" +
