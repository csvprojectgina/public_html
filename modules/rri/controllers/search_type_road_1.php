...
class Search_type_road extends Admin_Controller {
    ...
    public function grid_type_road() {
         ...
         */
        //$aColumns = array('pub_id', 'type_building', 'name_building', 'position_x', 'position_y', 'direction');
        $aColumns = array('road_number', 'road_name', 'type', 'length', 'width');
        ...
        if (isset($_GET['sSearch']) && $_GET['sSearch'] != "") {
            ...
            for ($i = 0; $i < count($aColumns); $i++) {
                if (isset($_GET['bSearchable_' . $i]) && $_GET['bSearchable_' . $i] == "true") {
                    $sWhere .= $aColumns[2] . " LIKE '%" . mysql_real_escape_string($_GET['sSearch']) . "%' OR ";
                }
            }
	...
	";
        ...
        if ($rResult->num_rows() > 0) {
            foreach ($rResult->result_array() as $aRow) {
                ...
                $row[] = '<a href="javascript:void(0)" class="editor_edit" style="text-align:center;">ព័ត៌. លំអិត</a>';
                $output['aaData'][] = $row;
            }
        }
