            ...
            <div class="panel panel-default">
                ...
                <div class="panel-heading">
                    <div class="row">
                        ...
                        <h3 class="panel-title"><!--<img src="<?= base_url('assets/bs/css/images/search.gif') ?>" />​ --><?= t('ព័ត៌មានភូមិបាល') ?></h3>
<!-- <button style="float: right; margin: -25px 0 0 0;" data-toggle="tooltip" data-placement="right" title="" type="submit" id="btn_print" class="btn btn-default" class="btn btn-default glyphicon glyphicon-file" id="btn_retires" type="submit"><?= t('រៀបចំបញ្ជីមន្ត្រីរាជការជា​PDF') ?></button> -->
                        ...
                        <div class="col-lg-6 col-sm-6 text-right">
                            <button class="btn btn-secondary btn-md"  data-toggle="modal" id="ajaxsgetdt" data-target="#addLandOfficials"><span class="glyphicon glyphicon-plus"> </span>បង្កើតភូមិបាល</button>
                        </div>
                    </div>
                ...
                <div class="panel-body">
                    <div style="margin: -10px 0 40px 0;vertical-align: middle;">
                        ...
                        <span style="float: right;">
                            <label for="search"><?= t('ស្វែងរកព័ត៌មានភូមិបាល') ?></label>
                    ...
                    <table cellpadding="0" cellspacing="0" border="1" class="table table-striped table-bordered table-hover" id="my_gr">
                        <thead>
                            <tr style="background-image: linear-gradient(to bottom,#f5f5f5 0,#e8e8e8 100%);background-repeat: repeat-x;">
                                <th style="text-align: left;" ><?= t('ព័ត៌មានការិយាល័យភូមិបាល') ?></th>
    ...
    <div class="modal fade" id="addLandOfficials" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
                    <h4 class="modal-title" id="myModalLabel"><span class="glyphicon glyphicon-plus"> </span> បញ្ជូលការិយាល័យភូមិបាលថ្មី</h4>
                </div>
                    ...
                    <div class="bs-example">
                        <form  class="form-horizontal f_save" role="form" method="post" id="f_save" >
                                ...
                                <div class="row">
                                    <div class="col-xs-12 col-sm-12">
                                        ...
                                        <a class="btn btn-default btn-select">
                                            <input type="hidden" class="btn-select-input" id="u_id" name="u_name" value="" />
                            ...
                            <div class="form-group">
                                <div class="row">
                                    <div class="col-xs-12 col-sm-12">
                                        <label for="inputEmail">បញ្ជូលការិយាល័យ</label>
                                        <a class="btn btn-default btn-select" >
                                            <input type="hidden" class="btn-select-input" id="office_id" name="office_name" value="" />
                                            <span class="btn-select-value">ជ្រើសរើសការិយាល័យ...</span>
                                            <span class='btn-select-arrow glyphicon glyphicon-chevron-down'></span>
                                            <ul id="displayoption_offices">
                                                <li class="selected">ជ្រើសរើសការិយាល័យ...</li>
                                            </ul>
                                        </a>
                                    </div>
                                </div>
                            ...
                            <div class="form-group">
                                <label for="inputEmail">បញ្ជូលភូមិបាល</label>
                                <input type="text" class="form-control" name="land_official" id="land_official" placeholder="បញ្ជូលការិយាល័យភូមិបាល" value="">
                            </div>
                            <div class="row">
...
//                    $('.ximage').show();
        ...
        $('body').delegate(".delete", "click", function (e) {
            ...
            if (id > 0) {
                if (confirm('តើអ្នកពិតជាលុប?')) {
                    $.ajax({
                        url: '<?= site_url('csv/csv_land_officials/delete') ?>',
                        type: 'POST',
                        dataType: 'html',
                        data: {id: id},
                        success: function (d) {
        ...
        $("body").delegate("#my_gr tbody tr", "click", function () {
            var id = $(this).data('id');
            if (id - 0 > 0) {
                window.location = "<?= site_url('csv/csv_land_officials/edit') ?>/" + id;
            } else {
    ...
    function grid(current_page, total_display) {
        ...
        $.ajax({
            url: '<?= site_url('csv/csv_land_officials/get_land_officials') ?>',
            type: 'post',
            datatype: 'json',
            // async: false,
            beforeSend: function () {
                ...
                if (record.length > 0) {
                    $.each(record, function (i, row) {
                        ...
                        tr += "<tr data-id='" + row.id + "'class='editor' target='_parent' >" +
                                "<td style='text-align: left;'>" + row.land_official + "</td>" +
...
//        option general deps
        $("#displayoption_offices").empty();
        // var i = '1';
        // var parameters = $('#f_save').serialize();
        $.ajax({
            url: '<?= site_url('csv/csv_land_officials/get_offices') ?>',
            type: 'POST',
            data: parameters,
            dataType: 'json',
            success: function (data) {
                jQuery.each(data, function (index, item) {
                    ...
                    //alert(index.unit);
                    console.log(item.unit);
                    $('#displayoption_offices').append('<li data-id=' + item.id + ' id ="office_id">' + item.office + '</li>');
                });
            },
            error: function () {
                alert('There was an error');
