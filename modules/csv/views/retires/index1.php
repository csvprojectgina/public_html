...
<script type="text/javascript">
   ...
   // edit by row ===========
    ...
    function grid(current_page, total_display) {
        ...
        $.ajax({
            url: '<?= site_url('csv/csv_retires/get_retires') ?>',
            type: 'post',
            datatype: 'json',
            // async: false,
            beforeSend: function() {
...
//                                " class='delete'>លុប</a></td>" +
                    ...
                    $('#my_gr tbody').html(tr);
                    $('#disp').html('' + (offset + 1) + ' ទៅ ' + ((offset + total_display) - 0 > total_record ? total_record : (offset + total_display)) + ' នៃទិន្នន័យ ' + total_record);
