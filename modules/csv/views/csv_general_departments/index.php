            ...
            <div class="panel panel-default">
                ...
                <div class="panel-heading">
                    <div class="row">
                        ...
                        <h3 class="panel-title"><!--<img src="<?= base_url('assets/bs/css/images/search.gif') ?>" />​ --><?= t('ព័ត៌មានអគ្គនាយកដ្ឋាន') ?></h3>
                ...
                <div class="panel-body">
                    <div style="margin: -10px 0 40px 0;vertical-align: middle;">
                        ...
                        <span style="float: right;">
                            <label for="search"><?= t('ស្វែងរកអគ្គនាយកដ្ឋាន') ?></label>
                    ...
                    <table cellpadding="0" cellspacing="0" border="1" class="table table-striped table-bordered table-hover" id="my_gr">
                        <thead>
                            <tr style="background-image: linear-gradient(to bottom,#f5f5f5 0,#e8e8e8 100%);background-repeat: repeat-x;">
                                <th style="text-align: left;width:70%;" ><?= t('អគ្គនាយកដ្ឋាន') ?></th>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel"><span class="glyphicon glyphicon-plus"> </span> បញ្ជូលអគ្គនាយកដ្ឋានថ្មី</h4>
                </div>
                <div class="modal-body">
                    <div class="bs-example">
                        <form  class="form-horizontal f_save" role="form" method="post" id="f_save" >
                            <div class="form-group">
                                <label for="inputEmail">បញ្ជូលអគ្គនាយកដ្ឋាន</label>
                                <input type="text" class="form-control" name="general_deps" id="inputEmail" placeholder="បញ្ជូលអគ្គនាយកដ្ឋាន" value="">
                            </div>
                            <div class="row">
...
<script type="text/javascript">
    $(function () {
        ...
        // save general  department===============
        $("#f_save").on('submit', function (e) {
            e.preventDefault();
//            return false;
            $.ajax({
                url: "<?= site_url('csv/csv_general_departments/insert_general_deps') ?>",
                type: "post",
                datatype: "json",
                beforeSend: function () {
...
//                    $('.ximage').show();
        ...
        $('body').delegate(".delete", "click", function (e) {
            ...
            if (id > 0) {
                if (confirm('តើអ្នកពិតជាលុប?')) {
                    $.ajax({
                        url: '<?= site_url('csv/csv_general_departments/delete') ?>',
                        type: 'POST',
                        dataType: 'html',
                        data: {id: id},
                        success: function (d) {
        ...
        // edit  general departments by row ===========
        $("body").delegate("#my_gr tbody tr", "click", function () {
            var id = $(this).data('id');
            if (id - 0 > 0) {
                window.location = "<?= site_url('csv/csv_general_departments/edit') ?>/" + id;
            } else {
    ...
    function grid(current_page, total_display) {
        ...
        $.ajax({
            url: '<?= site_url('csv/csv_general_departments/get_general_deps') ?>',
            type: 'post',
            datatype: 'json',
            // async: false,
            beforeSend: function () {
            ...
            success: function (data) {
                ...
                if (record.length > 0) {
                    $.each(record, function (i, row) {
                        // i++;
                        tr += "<tr data-id='" + row.general_dep_id + "'class='editor' target='_parent' >" +
                                "<td style='text-align: left;'>" + row.general_deps_name + "</td>" +
                                "<td style='text-align: center;'><a href='javascript: void(0)' data-id=" + row.general_dep_id +
