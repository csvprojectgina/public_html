...
<form class="form-horizontal" action="" role="form" method="post">
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-heading">
            ...
            <h3 class="panel-title"><!--<img src="<?= base_url('assets/bs/css/images/search.gif') ?>" />​ --><?= t('ព័ត៌មានមន្ត្រី') ?><a class="pull-right btn-xs btn-filter text-primary" href="<?= site_url('csv/csv_info/form') ?>"><i class="fa fa-plus-circle"></i> <?= t('បញ្ចូលថ្មី') ?></a></h3>
   ...
   // edit by row ===========
    ...
    function grid(current_page, total_display) {
        ...
        $.ajax({
            ...
            url: '<?= site_url('csv/csv_info/grid_csv_infok3') ?>',
                ...
                if (record.length > 0) {
                    ...
                    $.each(record, function(i, row) {
                        ...
                        tr += "<tr data-id='" + row.id + "'data-href='<?= site_url('csv/csv_info/edit') ?>/" + row.id + "'class='editor' target='_parent'>" +
                               ...
                               "<td style='text-align: left;'>" + row.salary_level + "</td>" +

                                 "<td style='text-align: left;'>" + row.unit_name + "</td>" +

//                                "<td style='text-align: left;'>" + row.work_office + "</td>" +
