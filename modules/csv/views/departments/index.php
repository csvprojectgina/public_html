            ...
            <div class="panel panel-default">
                ...
                <div class="panel-heading">
                    <div class="row">
                        ...
                        <h3 class="panel-title"><!--<img src="<?= base_url('assets/bs/css/images/search.gif') ?>" />​ --><?= t('ព័ត៌មាននាយកដ្ឋាន') ?></h3>
<!-- <button style="float: right; margin: -25px 0 0 0;" data-toggle="tooltip" data-placement="right" title="" type="submit" id="btn_print" class="btn btn-default" class="btn btn-default glyphicon glyphicon-file" id="btn_retires" type="submit"><?= t('រៀបចំបញ្ជីមន្ត្រីរាជការជា​PDF') ?></button> -->
                        ...
                        <div class="col-lg-6 col-sm-6 text-right">
                            <button class="btn btn-secondary btn-md"  data-toggle="modal" id="ajaxsgetdt" data-target="#addDepartment"><span class="glyphicon glyphicon-plus"> </span> បង្កើតនាយកដ្ឋាន</button>
                        </div>
                    </div>
                    ...
                    <table cellpadding="0" cellspacing="0" border="1" class="table table-striped table-bordered table-hover" id="my_gr">
                        <thead>
                            <tr style="background-image: linear-gradient(to bottom,#f5f5f5 0,#e8e8e8 100%);background-repeat: repeat-x;">
                                ...
                                <th style="text-align: left;" ><?= t('ព័ត៌មានអគ្គនាយកដ្ឋាន') ?></th>
                ...
                <div class="modal-body">
                    <div class="bs-example">
                        <form  class="form-horizontal f_save" role="form" method="post" id="f_save" >
                            ...
                            <div class="form-group">
                                <label for="inputEmail">បញ្ជូលនាយកដ្ឋាន</label>
                                <input type="text" class="form-control" name="d_name" id="d_name" placeholder="បញ្ជូលនាយកដ្ឋាន" value="">
                            </div>
                            <div class="row">
...
//            return false;
            $.ajax({
                url: "<?= site_url('csv/csv_departments/insert_departments') ?>",
                type: "post",
                datatype: "json",
                beforeSend: function () {
...
//                    $('.ximage').show();
        ...
        $('body').delegate(".delete", "click", function (e) {
            ...
            if (id > 0) {
                if (confirm('តើអ្នកពិតជាលុយ?')) {
                    $.ajax({
                        url: '<?= site_url('csv/csv_departments/delete') ?>',
                        type: 'POST',
                        dataType: 'html',
                        data: {id: id},
                        success: function (d) {
        ...
        $("body").delegate("#my_gr tbody tr", "click", function () {
            var id = $(this).data('id');
            if (id - 0 > 0) {
                window.location = "<?= site_url('csv/csv_departments/edit') ?>/" + id;
            } else {
    ...
    function grid(current_page, total_display) {
        ...
        $.ajax({
            url: '<?= site_url('csv/csv_departments/load') ?>',
            type: 'post',
            datatype: 'json',
            // async: false,
            beforeSend: function () {
            ...
            success: function (data) {
                ...
                if (record.length > 0) {
                    $.each(record, function (i, row) {
                        // i++;
                        tr += "<tr data-id='" + row.d_id + "'class='editor' target='_parent' >" +
                                ...
                                "<td style='text-align: center;'><a href='javascript: void(0)' data-id=" + row.d_id +
