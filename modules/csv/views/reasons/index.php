            ...
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  ...
                  <h4 class="modal-title" id="myModalLabel"><span class="glyphicon glyphicon-plus"> </span> បញ្ជូលមូលហេតុថ្មី</h4>
                </div>
                <div class="modal-body">
									<div class="bs-example">
								    <form  class="form-horizontal f_save" role="form" method="post" id="f_save" >
								        <div class="form-group">
								            <label for="inputEmail">បញ្ជូលមូលហេតុ</label>
								            <input type="text" class="form-control" name="reason_name" id="inputEmail" placeholder="បញ្ជូលមូលហេតុ" value="">
								        </div>
...
//            return false;
            $.ajax({
                url: "<?= site_url('csv/csv_reasons/save') ?>",
                type: "post",
                datatype: "json",
                beforeSend: function() {
...
//                    $('.ximage').show();
        ...
        $('body').delegate(".delete", "click", function(e) {
            var rea_id = $(this).data('id') - 0;
            var tr = $(this).parent().parent();
            if (rea_id > 0) {
                if (confirm('តើអ្នកពិតជាលុយ?')) {
                    $.ajax({
                        url: '<?= site_url('csv/csv_reasons/delete') ?>',
                        type: 'POST',
                        dataType: 'html',
                        data: {rea_id: rea_id},
                        success: function(d) {
   ...
   // edit by row ===========
        $("body").delegate("#my_gr tbody tr", "click", function() {
            var id = $(this).data('id');
            if (id - 0 > 0) {
                window.location = "<?= site_url('csv/csv_reasons/edit') ?>/" + id;
            } else {
    ...
    function grid(current_page, total_display) {
        ...
        $.ajax({
            url: '<?= site_url('csv/csv_reasons/load') ?>',
            type: 'post',
            datatype: 'json',
            // async: false,
            beforeSend: function() {
            ...
            success: function(data) {
                ...
                if (record.length > 0) {
                    $.each(record, function(i, row) {
                        // i++;
                        tr += "<tr data-id='" + row.rea_id + "'class='editor' target='_parent' >" +
                                "<td style='text-align: left;'>" + row.reason_name + "</td>" +
                                "<td style='text-align: center;'><a href='javascript: void(0)' data-id=" + row.rea_id +
