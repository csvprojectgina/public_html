...
<div class="panel panel-default panel-info">
    <div class="panel-heading">
        <h3 class="panel-title"> <?= t('ទំរង់មន្ត្រី ឡើងថ្នាក់ និង ឋានន្តរស័ក្តិតាមវេនឬ តាមអតីតការងារ') ?></h3>
    </div>
    ...
    <form class="form-horizontal"  role="form"  id="frm_former_work">
            ...
            <div class="panel panel-info">
                <div class="panel-heading">
                    <h3 class="panel-title">
                        <a href="#collapseone" style="color: #797979 !important; margin-left: -13px; margin-top: 3px;" ><?= t('បច្ចុប្បន្នភាព មន្ត្រីឡើងថ្នាក់និង ឋានន្តរស័ក្តិតាមវេនឬ អតីតការងារ') ?></a>
                    </h3>
                </div>
...
<script type="text/javascript">
    ...
    function save_promote_certificate() {
        ...
        if (txtdate === '') {
            ...
            $.ajax({
                type: 'post',
                url: '<?= site_url('csv/csv_update/promoted_former_work') ?>',
                datatype: 'json',
                data: {
                    csv_id: csv_id,
                ...
                success: function (data) {
                    ...
                    if (data.status === 'success') {
                        swal({
                            text: "មន្ត្រី ត្រូវឡើងថ្នាក់ និង ឋានន្តរស័ក្តិតាមវេនឬ តាមអតីតការងារ ត្រូវបានរក្សាទុកដោយជោគជ័យ",
                            type: 'success',
                        ...
                        setTimeout(function () {
                            window.location.href = "<?php echo site_url('csv/csv_update/csv_promoted_former_work') ?>/";
                        }, 2500);
                    }
                }
            });
    ...
    $(document).ready(function () {
        ...
        $.typeahead({
            ...
            delay: 500,
            hint: true,
            href: '<?php echo site_url('csv/csv_update/get_csv_detail?value={{id}}-{{name}}&id={{idtable}}&frm=former_work ') ?>',
            cancelButton: true,
            display: ["id", "name"],
            backdrop: {
