...
<div class="panel panel-default">
    <div class="panel-heading thumbnail btn-group">
        <h3 class="panel-title">
            ...
            <?= t('ស្ថិតិមន្រ្តីរាជការ តម្លើងឋានន្តរសក្កិតាម វេន/អតីតការងារ') ?>
        </h3>
    </div>
    ...
    <form class="form-horizontal" role="form" action="<?= site_url('csv/csv_report/pdf') ?>" method="post">
        ...
        <table cellpadding="0" cellspacing="0" border="1"
            ...
            <tr style="background-image: linear-gradient(to bottom,#f5f5f5 0,#e8e8e8 100%);background-repeat: repeat-x;">
                ...
                <th style="text-align: center; vertical-align: middle;"><?= t('កាលបរិច្ឆេទតម្លើង') ?></th>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $.ajax({
            url: '<?= site_url('csv/csv_promoted_former_work/get_csv_promoted_former_work') ?>',
            type: 'post',
            datatype: 'json',
            beforeSend: function () {
    ...
    $(document).on('click', '#btn_print', function (e) {
        ...
        var by_year = $('#by_year').val();
        var win = window.open('<?= site_url('csv/csv_promoted_former_work/former_work_pdf?by_year=') ?>'+by_year, '_blank');
        if (win) {
