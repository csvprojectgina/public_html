    ...
    <form class="form-horizontal" role="form" action="<?= site_url('csv/csv_report/pdf') ?>" method="post">
        ...
        <table cellpadding="0" cellspacing="0" border="1"
            ...
            <tr style="background-image: linear-gradient(to bottom,#f5f5f5 0,#e8e8e8 100%);background-repeat: repeat-x;">
                ...
                <th style="text-align: center; vertical-align: middle;"><?= t('សាលា') ?></th>
                <th style="text-align: center; vertical-align: middle;"><?= t('ថ្ងៃខែឆ្នាំកំណើត') ?></th>
                <th style="text-align: center;  vertical-align: middle;"><?= t('មុខតំណែង') ?></th>
                <th style="text-align: center; vertical-align: middle;"><?= t('កាលបរិច្ឆេទចូល') ?></th>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $.ajax({
            url: '<?= site_url('csv/internship_info/get_intern_report') ?>',
            type: 'post',
            datatype: 'json',
            beforeSend: function () {
    ...
    $(document).on('click', '#btn_print', function (e) {
        ...
        var by_year = $('#by_year').val();
        var win = window.open('<?= site_url('csv/internship_info/intern_report_pdf?by_year=') ?>'+by_year, '_blank');
        if (win) {
