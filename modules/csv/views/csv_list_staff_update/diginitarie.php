...
<div class="panel panel-default">
    <div class="panel-heading thumbnail btn-group">
        <h3 class="panel-title">
            <!--<img src="<?= base_url('assets/bs/css/images/search.gif') ?>" />? --><?= t('ស្ថិតិមន្រ្តីរាជការទទួលបានមេដាយនិងគ្រឿងឥស្សរិយយស') ?></h3>
    </div>
    ...
    <form class="form-horizontal" role="form" action="<?= site_url('csv/csv_report/pdf') ?>" method="post">
        ...
        <table cellpadding="0" cellspacing="0" border="1"
            ...
            <tr style="background-image: linear-gradient(to bottom,#f5f5f5 0,#e8e8e8 100%);background-repeat: repeat-x;">
                ...
                <th style="text-align: center; vertical-align: middle;"><?= t('ប្រភេទមេដាយ') ?></th>
                <th style="text-align: center; vertical-align: middle;"><?= t('ថ្នាក់នៃមេដាយ') ?></th>
                <th style="text-align: center; vertical-align: middle;"><?= t('កាលបរិច្ឆេ') ?></th>
            </tr>
            </thead>
            <tbody>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $.ajax({
            url: '<?= site_url('csv/csv_list_staff_update/get_staff_diginitarie') ?>',
            type: 'post',
            datatype: 'json',
            beforeSend: function () {
// $('.xmodal').show();
...
// $('.xmodal').show();
    ...
    $(document).on('click', '#btn_print', function (e) {
        ...
        var by_year = $('#by_year').val();
        var win = window.open('<?= site_url('csv/csv_list_staff_update/get_diginitarie_report_byPdf?by_year=') ?>'+by_year, '_blank');
        if (win) {
