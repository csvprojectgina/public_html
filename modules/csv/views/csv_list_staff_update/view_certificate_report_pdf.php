...
$mpdf = new mPDF();
$title = 'ស្ថិតិមន្ត្រី ឡើងថ្នាក់ និង ឋានន្តរស័ក្តិតាមវិញានបនប័ត្រ';
...
$strsql = "SELECT  cs.id,
                    ...
                    LEFT JOIN salary AS sa ON sa.id = w.id
                    LEFT JOIN salary_history AS sh ON sh.id = cs.id
                    LEFT JOIN tbl_csvupdate as su ON su.csv_id=cs.id WHERE su.is_type = 'Officers were promoted by certificate'";
...
$body = '<table border="0" cellpadding="3" cellspacing="0" style="border-collapse: collapse;font-family: khmermef1; width: 100%; text-align: center;">
           <thead>
            <tr style="background-color: lightgrey;">
                ...
                <th style="border: 1px solid blue; font-family: KhmerOS; font-size:11px ">ក្របខណ្ឌចាស់</th>
                <th style="border: 1px solid blue; font-family: KhmerOS; font-size:11px ">ក្របខណ្ឌថ្មី</th>
                <th style="border: 1px solid blue; font-family: KhmerOS; font-size:11px ">កាលបរិច្ឆេទឡើងថ្នាក់</th>
    ...
    if ($getleader->num_rows() > 0) {
        ...
        foreach ($getleader->result() as $row) {
            $body .= '<tr>
                        ...
                        <td style="height: 30px;">' . $row->old_salary . '</td>
                        <td style="height: 30px;">' . $row->salary_level . '</td>
