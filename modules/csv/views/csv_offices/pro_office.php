            ...
            <div class="panel panel-default">
                ...
                <div class="panel-heading">
                    <div class="row">
                        ...
                        <h3 class="panel-title"><!--<img src="<?= base_url('assets/bs/css/images/search.gif') ?>" />​ --><?= t('ព័ត៌មានការិយាល័យរាជធានី / ខេត្ត') ?></h3>
                ...
                <div class="modal-body">
                    <div class="bs-example">
                        <form  class="form-horizontal f_save" role="form" method="post" id="f_save" >
                            ...
                            <div class="form-group">
                                <label for="inputEmail">បញ្ជូលការរិយាល័យរាជធានី / ខេត្ត</label>
                                <input type="text" class="form-control" name="province_office" id="province_office" placeholder="បញ្ជូលការរិយាល័យ" value="">
                            </div>
                            <div class="row">
...
//            return false;
            $.ajax({
                url: "<?= site_url('csv/csv_offices/insert_pro_office') ?>",
                type: "post",
                datatype: "json",
                beforeSend: function () {
...
//                    $('.ximage').show();
        ...
        $('body').delegate(".delete", "click", function (e) {
            var pro_office_id = $(this).data('id') - 0;
             //alert(pro_office_id);
            ...
            if (pro_office_id > 0) {
                if (confirm('តើអ្នកពិតជាលុប')) {
                    $.ajax({
                        ...
                        type: 'POST',
                        dataType: 'html',
                        data: {pro_office_id: pro_office_id},
                        success: function (d) {
                            //alert(d);
                            tr.remove();
                            location.reload();
        ...
        $("body").delegate("#my_gr tbody tr", "click", function () {
            var pro_office_id = $(this).data('id');
            if (pro_office_id - 0 > 0) {
                window.location = "<?= site_url('csv/csv_offices/edit_pro_office') ?>/" + pro_office_id;
            } else {
    ...
    function grid(current_page, total_display) {
        ...
        $.ajax({
            url: '<?= site_url('csv/csv_offices/load_pro_office') ?>',
            type: 'post',
            datatype: 'json',
            // async: false,
            beforeSend: function () {
