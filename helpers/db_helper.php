...
// opt1. ================================
if (!function_exists('getoption1')) {

    function getoption1($sql, $key, $display, $data_hide = '', $select = '', $istop = false) {
        $ci = & get_instance();
        $query = $ci->db->query($sql);
        $op = "";
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $row) {
                $op .= '<option ' . ($select == $row[$key] ? ' selected="selected" ' : '') . ' value="' . $row[$key] . '" data-dis_name="' . $row[$data_hide] . '">' . $row[$display] . '</option>';
            }
        }
        return $op;
    }
